...
from flexgen.client.remote_sequential import RemoteSequential #*** modified
...
class DistributedLlamaModel(FromPretrainedMixin, PTuneMixin, LlamaModel):
    ...
    def __init__(self, flexconfig: Union[str, llamaConfig],env: ExecutionEnv,path: str,policy: Policy, config: DistributedLlamaConfig, *, dht: Optional[hivemind.DHT] = None):
        ...
        self.layers = RemoteSequential(flexconfig, env, policy, config, dht=dht) # create RemoteSequential instance to manage distributed layers

        self.requires_grad_(False)  # Forbid accumulate grads for embeddings and layernorm
        self.init_prompts(config)
        
        ##--------------------------------------------------------------------------------###
        if isinstance(flexconfig, str):
            flexconfig = get_opt_config(flexconfig)
